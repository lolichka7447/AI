...
export class AdminCalendarPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.calendarGrid = page.locator('[class*="calendar"], table').first();
...
export class AdminExportPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.projectFilter = page.locator('[class*="project-filter"], select:has-text("Проект")').first();
  }
...
export class AccountSettingsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.trackerTable = page.locator('table').first();
    this.trackerRows = this.trackerTable.locator('tbody tr');
    this.addTrackerButton = page.getByRole('button', { name: /Добавить|Add/i }).first();
    this.trackerSearchInput = page.locator('input[placeholder*="Поиск" i], input[class*="search"]').first();
    this.saveButton = page.getByRole('button', { name: /Сохранить|Save/i }).first();
    ...
    this.trackerModal = page.locator('[class*="modal"], [role="dialog"]').first();
    this.trackerUrlInput = this.trackerModal.locator('input[placeholder*="URL" i], input[class*="url"]').first();
    this.trackerLoginInput = this.trackerModal.locator('input[placeholder*="логин" i], input[class*="login"]').first();
    this.trackerPasswordInput = this.trackerModal.locator('input[type="password"]').first();
    this.trackerSaveButton = this.trackerModal.locator('button:has-text("Сохранить"), button[type="submit"]').first();
    this.trackerDeleteButton = page.getByRole('button', { name: /Удалить|Delete/i }).first();
  }
